...
#### 多容器互通（别的 docker 容器调 tido）

别为了让别的容器能访问就 `0.0.0.0` 全开。正解是 docker 共享网络，host 暴露面 = 0：
...
`tido` 的 compose 已经声明加入 `mcp-net`（`external: true`）。让你的其它 compose 项目也加入同一个网络：
...
那个容器内直接访问 `http://tido:8080/mcp`（用容器名做 DNS，端口是容器内端口 8080，不是 host 上的 18080）。
...
```
cmd/tido/main.go            # MCP server 入口（stdio + HTTP transport）
cmd/tido/http.go            # HTTP transport + bearer auth 中间件
internal/
